...
\begin{frame}{帕累托前沿分析：实枪实测数据 (M4A1)}
    \textbf{核心概念：}帕累托前沿代表了非支配最优解的边界——在不牺牲其他属性的前提下，没有任何单项属性可以被进一步提升。
    ...
    \begin{tikzpicture}[scale=0.88]
        ...
        % Axes
        \draw[->, thick] (0.5,0.4) -- (7.0,0.4) node[right] {\small 人机工效 ($E$)};
        \draw[->, thick] (0.5,0.4) -- (0.5,3.8) node[above] {\small 垂直后坐力 ($R$，越低越好)};
        ...
        \node[font=\footnotesize, red!70!black, align=center] at (2.2, 3.1) {\textbf{受支配区域 (被包络)}\\(非最优改装配置)};

        \node[font=\footnotesize, gray!80, align=center] at (5.0, 0.8) {\textit{物理不可达区域}};
        ...
        % Highlighted builds
        \node[fill=red!80!black, star, star points=5, inner sep=2.2pt, label={[font=\tiny, red!80!black]above:极限后坐 (47)}] at (1.13, 0.67) {};
        \node[fill=green!60!black, star, star points=5, inner sep=2.2pt, label={[font=\tiny, green!60!black]below right:甜点均衡 (73人机 / 53后坐)}] at (3.73, 1.24) {};
        \node[fill=purple!80!black, star, star points=5, inner sep=2.2pt, label={[font=\tiny, purple!80!black]right:极限人机 (100)}] at (6.07, 2.28) {};
        ...
        \node[right, font=\tiny] at (1.0, 2.5) {真实帕累托前沿 (数学最优)};
        \node[fill=gray!60, circle, inner sep=1.2pt] at (0.9, 2.1) {};
        \node[right, font=\tiny] at (1.0, 2.1) {随机有效改装 (劣解样本)};
    \end{tikzpicture}
    \end{center}
    \vspace{-0.2cm}
    \textbf{实战启示：}点云内部的配置存在严重的性能浪费（相同人机却高出十几点后坐力）。优化器在数学上保证玩家始终位于蓝色最优边界。
\end{frame}
