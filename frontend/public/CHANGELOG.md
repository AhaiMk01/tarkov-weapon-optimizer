...
### Added
- **Weapon gallery picker**: The sidebar's two filter dropdowns and searchable list are replaced by a modal card grid — grouped by caliber or class, searchable, with the category/caliber filters moved inside it. Each group header has a Select all (confirmed past 12 weapons). The sidebar now shows the current pick, which is itself the control that opens the gallery; Explore shows its picks as a compact square tile tray with a Clear all.
- **Build detail from Explore**: Clicking a point on the trade-off chart pins a panel with that build's attachments, prices, and icons. The results list gained a per-point "View build" opening the full manifest in a drawer. The data was always present — an ExplorePoint carries its whole item list — it just had nowhere to be shown.
- **Run cancellation**: An in-flight multi-weapon comparison can be stopped. Points already collected stay on screen. The in-flight weapon still finishes, since HiGHS has no abort channel.

### Changed
- **No cap on compared weapons**: Explore was limited to six, enforced both in the UI and again as a silent truncation at solve time. Both are gone.
- **Explore results restructured**: One expandable row per weapon instead of one per frontier point — 16 weapons went from 120 rows across 8 pages to 16 rows with no pagination — showing each weapon's frontier size, best ergo, lowest recoil, and cheapest price. The list collapses, and when collapsed its header carries the weapon swatches, replacing the chart legend that clipped at 15+ series. The plot is measured from the panel's remaining space, so collapsing the list gives the room to the chart.
- **Caliber names**: `545x39` and `1143x23ACP` become `5.45x39` and `.45 ACP`, ordered sensibly rather than by string collation, which sorted 545 after 127.

### Fixed
- **Duplicate series colours past six weapons**: The chart palette held exactly six colours and wrapped, so weapon 7 drew in weapon 1's colour once the cap was lifted. Colours beyond six are now generated by farthest-point selection in CIE HCL, keyed to a weapon's position in the run so they stay stable as results stream in. Closest pair on the worst case improved from ΔE 8.0 to 19.5 at 24 series, with gamut clipping down from 9 entries to 0.
- **Cancel unreachable before the first weapon returned**: The empty-results view had a loading-locked Solve and no Cancel, so the longest wait in a run — and an all-infeasible run — could not be stopped.
- **Cancelled vs superseded runs**: A superseded run no longer writes its stale results over the run that replaced it.
- **Pinned build panel**: Survives the streaming updates of a multi-weapon run instead of closing on each one, and clears when a resize moves the plot out from under it.
- **Select all confirmation** rendered in default light on a dark app, via antd's unthemed static modal.
